...
3. make sure the release tag you plan to use starts with `v`, for example `v0.1.0`
4. run `cabal test`
...
Open the `Release` workflow in GitHub Actions and provide the values for the release you are about to ship, for example the first release `v0.1.0`:
